...
The bindings are built from this repo: [mopro-zkemail-nr](https://github.com/zkemail/mopro-zkemail-nr)
...
command. Or run the binary with

```sh
CONFIGURATION=release cargo run --bin ios IOS_ARCHS=aarch64-apple-ios,aarch64-apple-ios-sim,x86_64-apple-ios
```

and the bindings are uploaded here: [`MoproiOSBindings.zip`](https://github.com/zkmopro/mopro-zkemail-nr/releases/download/v0.1.0/MoproiOSBindings.zip)
...
Then open the `Tests/UniTest.swift` in Xcode and choose a simulator/device.
